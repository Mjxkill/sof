# V5.4.1 E5.e.2-step1 — capture 8ch native (1 comp eq_iir + 1 comp drc + 1 comp pga)
#
...
#               -> B1(8ch) -> drc(8ch, blob single-config commun aux 8 voies)
#               -> B2(8ch) -> pga(8ch, 8 controls indép via channel-map)
#               -> B3(8ch) -> host PCM 0
#
# Controls:
#   - 1 control bytes "EQ IIR Coefs" (blob avec assign_response[8]={0..7} ou {-1×8} pass)
#   - 1 control bytes "DRC Config"   (blob single-config — partagé aux 8 voies, sera per-channel via patch D3 plus tard)
#   - 8 control mixers "Strip[1-8] Volume" (un par channel position FL/FR/RL/RR/FC/LFE/SL/SR)
#
# Trade-off Step 1:
#   - DRC partagé (limite intrinsèque sans patch SOF). User a accepté validation archi avant patch.
#   - 8 vols indép par channel ALSA standard. Si l'user veut L+R cascade per strip = E5.e.3.
#
# Step 1 valide :
#   - eq_iir 8ch multi-response fonctionnel
#   - pga 8ch channel-map fonctionnel
#   - Architecture 1-comp-8ch tient en CPU/heap (vs 8-instances E5.e.2-multi-comp qui a échoué EIO)
#
...
#
P_GRAPH(pipe-eq-drc-pga-8ch-capture, PIPELINE_ID,
	LIST(`		',
